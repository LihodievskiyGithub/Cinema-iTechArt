...
<div id="wrapper">
	<form  @submit.prevent id="signin" method="" action="" autocomplete="off">
		<input type="text" id="user" name="user" placeholder="email" v-model="email" />
		<input type="password" id="pass" name="pass" placeholder="password" v-model="password" />
		<button @click="login" type="submit">&#xf0da;</button>
		{{ error }}
	</form>
</div>
...
#signin input:last-of-type::before{
		display: block;
	position: relative;
	height: 2px;
	background: rgb(52, 56, 61);
